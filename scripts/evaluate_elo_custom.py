'''
Evaluate ELO using custom rating system
'''
import chess
import chess.pgn
import pexpect
...
if __name__ == "__main__":
    ...
    # Linux
    bots = [StockfishBot(f"Stockfish-{i}", i, "engines/stockfish-ubuntu-x86-64-avx2") for i in range(1, 6)]
    bots = [StockfishBot(f"Stockfish-{2}", 2, "engines/stockfish-ubuntu-x86-64-avx2")]
    bots += [MinimaxBot("Mybot", "src/cpp/agent")]

    intermediate_path = os.path.join("output", f"evaluate_elo_custom-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}")
    os.makedirs(intermediate_path, exist_ok=True)
    
    generate_stockfish_sample_matches(bots, 
                                      2, 
                                      os.path.join(intermediate_path, "history.pgn"),
                                      os.path.join(intermediate_path, "results.csv"),
                                      os.path.join(intermediate_path, "elo.csv")
                                      )